...
$tmp = get_absolute_path($tmp_dir . '/' .(isset($_REQUEST['file'])? $_REQUEST['file'] : ''));
...
if(isset($_GET['do']) == 'list') {
	...
	exit;
} elseif (isset($_POST['do']) == 'delete') {
	if($allow_delete) {
		rmrf($file);
	}
	exit;
} elseif (isset($_POST['do']) == 'mkdir' && $allow_create_folder) {
	...
	@mkdir($_POST['name']);
	exit;
} elseif ( isset($_POST['do']) == 'upload' && $allow_upload) {
